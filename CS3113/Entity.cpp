    ...
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {
            ...
            if (mVelocity.y > 0) 
            {
                ...
                if (collidableEntities->mEntityType == ENEMY)
...
void Entity::AIWander(Map *map) { 
    ...
    float frontOffset = halfWidth + 2.0f;
    ...
    if (mDirection == LEFT)
    {
        ...
        wallProbe = { mPosition.x - frontOffset, mPosition.y };
        floorProbe = { mPosition.x - frontOffset, mPosition.y + halfHeight + 2.0f };
    }
    else
    {
        moveRight();
    ...
    bool wallAhead = map->isSolidTileAt(wallProbe, &xOverlap, &yOverlap);
    bool groundAhead = map->isSolidTileAt(floorProbe, &xOverlap, &yOverlap);

    if (wallAhead || !groundAhead)
    {
        if (mDirection == LEFT) setDirection(RIGHT);
...
{
    float directionX = target->getPosition().x - mPosition.x;
    float directionY = target->getPosition().y - mPosition.y;

    float chaseRadiusX = 250.0f;
    float chaseRadiusY = 120.0f;

    if (fabs(directionX ) < chaseRadiusX && fabs(directionY) < chaseRadiusY)
    {
        if (directionX  < 0.0f)
        {
            moveLeft();
        ...
        mMovement.x *= 0.45f;   // slower than normal
    }
    else
    {
        mMovement.x = 0.0f;
    ...
    Entity *collidableEntities, int collisionCheckCount)
{
    ...
    if (mEntityType == NPC || mEntityType == ENEMY) AIActivate(player, map);
